...
public class GridFactory {
  ...
  //Recibe un vector de enteros y el �ndice del primer y �ltimo elemento v�lido del mismo
    ...
    /********************************************************************************
     * Añadida por PEDRO, para el manejo de la rejilla
     * es llamada desde Game.java despues de cargar todas las escenas
     ********************************************************************************/
    
    public static void setGrill(Chapter currentChapter) {
        //System.out.println("Calculando la rejilla....");         
        
        //En primer lugar obtenemos el listado de las escenas
        
        List<Scene> Scenes = new ArrayList<Scene>( );
        List<ActiveArea> ActiveAreas = new ArrayList<ActiveArea>( );
        List<Exit> Exits = new ArrayList<Exit>( );
        List<ElementReference> ElementReferences = new ArrayList<ElementReference>( );
        ...
        Scenes = currentChapter.getScenes( );

        //Recorremos cada una de las escenas y sacamos las posiciones de los objetos
        for( int i = 0; i < Scenes.size( ); i++ ) {
            // Procedemos a buscar las posiciones de cada objetos que haya en la imagen
            // En principio nos da igual el tipo de objeto que sea. 
            
            scene = Scenes.get( i ); 
            
            
            String IdScene = scene.getId( );
            boolean repeat=false;
            // Procedemos al analisis de las salidas.
            Exits = scene.getExits( );
            ...
            if(!Exits.isEmpty( )) {
                for( int j = 0; j < Exits.size( ); j++ ) {
                    ...
                    position.setIdScene(scene.getId( ));
                    position.setType("Exit");
                    position.setX( exit.getX( ),exit.getWidth( ));
                    position.setY( exit.getY( ),exit.getHeight( ));
                    for (int k=0; k<Positions.size( );k++){
                        if (position.getIdScene( ).equals( Positions.get( k ).getIdScene( ) )
                                && position.getType( ).equals( Positions.get( k ).getType( ) )
                                && position.getX( )== Positions.get( k ).getX( )
                                && position.getY( )== Positions.get( k ).getY( )){
                            repeat=true;    
                        }
                    }
                    if (!repeat) {
                   ...
                   // System.out.println(index + " Salida con centro " + position.getX( )+ " " + position.getY( ) + " de la escena " + position.getIdScene( ));
                   // System.out.println(index + " x " + exit.getX( )+ " y " + exit.getY( ) + " ancho " + exit.getWidth( )+ " alto " + exit.getHeight( ));
                
                }   
            }
            
            // Procedemos al analisis de las areas activas.
            ActiveAreas = scene.getActiveAreas( );
        
            if(!ActiveAreas.isEmpty( )) {
                for( int j = 0; j < ActiveAreas.size( ); j++ ) {
                    activearea = ActiveAreas.get( j );
                    repeat=false;
                    position = new GridPosition("",0,0,"","");
                    position.setIdScene(IdScene);
                    position.setType("ActiveArea");
                    position.setIdElement(activearea.getId( ));
                    position.setX( activearea.getX( ),activearea.getWidth( ));
                    position.setY( activearea.getY( ),activearea.getHeight( ));
                    for (int k=0; k<Positions.size( );k++){
                        if (position.getIdScene( ).equals( Positions.get( k ).getIdScene( ) )
                                && position.getType( ).equals( Positions.get( k ).getType( ) )
                                && position.getX( )== Positions.get( k ).getX( )
                                && position.getY( )== Positions.get( k ).getY( )
                                && position.getIdElement( ).equals( Positions.get( k ).getIdElement())){
                            repeat=true;    
                        }
                    }
                    if (!repeat) {
                 ...
                 //   System.out.println(index + " Area activa con centro " + position.getX( )+ " " + position.getY( ) + " de la escena " + position.getIdScene( ));
                  //  System.out.println(index + " x " + activearea.getX( )+ " y " + activearea.getY( ) + " ancho " + activearea.getWidth( )+ " alto " + activearea.getHeight( ));
                }   
            }
           
            ElementReferences = scene.getItemReferences( );
            
            if(!ElementReferences.isEmpty( )) {
              
                for( int j = 0; j < ElementReferences.size( ); j++ ) {
                    elementreference = ElementReferences.get( j );

                    repeat=false;
                    position = new GridPosition("",0,0,"","");
                    position.setIdScene(IdScene);
                    position.setType("ItemReferences");
                    position.setIdElement(elementreference.getTargetId( ));
                    position.setX( elementreference.getX( ),0);
                    position.setY( elementreference.getY( ),0); 
                    for (int k=0; k<Positions.size( );k++){
                        if (position.getIdScene( ).equals( Positions.get( k ).getIdScene( ) )
                                && position.getType( ).equals( Positions.get( k ).getType( ) )
                                && position.getX( )== Positions.get( k ).getX( )
                   ...
                   //System.out.println("Item Reference " + position.getX( )+ " " + position.getY( ) + " con identificador " + elementreference.getTargetId( ));   
                }   
            }
            ...
            if(!ElementReferences.isEmpty( )) {
                for( int j = 0; j < ElementReferences.size( ); j++ ) {
                    ...
                    position = new GridPosition("",0,0,"","");
                    position.setIdScene(IdScene);
                    position.setType("AtrezzoReferences");
                    position.setIdElement(elementreference.getTargetId( ));
                    position.setX( elementreference.getX( ),0);
                    position.setY( elementreference.getY( ),0);
                    for (int k=0; k<Positions.size( );k++){
                    ...
                    //System.out.println(index + " Atrezzo Reference " + position.getX( )+ " " + position.getY( ) + " con identificador " + elementreference.getTargetId( ));
                }   
            }
            
            ElementReferences = scene.getCharacterReferences( );
            ...
            if(!ElementReferences.isEmpty( )) {
                for( int j = 0; j < ElementReferences.size( ); j++ ) {
                    ...
                    position = new GridPosition("",0,0,"","");
                    position.setIdScene(IdScene);
                    position.setType("CharacterReferences");
                    position.setIdElement(elementreference.getTargetId( ));
                    position.setX( elementreference.getX( ), 0);
                    position.setY( elementreference.getY( ), 0);
                    for (int k=0; k<Positions.size( );k++){
                    ...
                    //System.out.println(index + " character Reference " + position.getX( )+ " " + position.getY( ) + " con identificador " + elementreference.getTargetId( ));
                   
                }
            }
            
        }
        GridFactory.oldGrid=Positions;
        //System.out.println("El numero total de objetos es " + index);
