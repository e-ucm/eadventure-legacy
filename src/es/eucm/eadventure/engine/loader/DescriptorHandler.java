...
public class DescriptorHandler extends DefaultHandler {
    ...
    public void startElement( String namespaceURI, String sName, String qName, Attributes attrs ) throws SAXException {
        ...
        if( qName.equals( "gui" ) ) {
            int guiType = GameDescriptor.GUI_TRADITIONAL;
            ...
            for( int i = 0; i < attrs.getLength( ); i++ ) {
                ...
                if( attrs.getQName( i ).equals( "type" ) ) {
                    if( attrs.getValue( i ).equals( "traditional" ) )
                        guiType = GameDescriptor.GUI_TRADITIONAL;
                    else if( attrs.getValue( i ).equals( "contextual" ) )
                        guiType = GameDescriptor.GUI_CONTEXTUAL;
                }
            ...
            for( int i = 0; i < attrs.getLength( ); i++ ) {
                if (attrs.getQName( i ).equals( "playerTransparent" )){
                    if (attrs.getValue( i ).equals( "yes" )){
                        gameDescriptor.setPlayerMode(GameDescriptor.MODE_PLAYER_1STPERSON);
                    }
                    else if(attrs.getValue( i ).equals( "no" )){
                        gameDescriptor.setPlayerMode(GameDescriptor.MODE_PLAYER_3RDPERSON);
                    }
                }
            }
        }
