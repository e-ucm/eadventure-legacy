package es.eucm.eadventure.adventureeditor.control.controllers.timer;
...
import es.eucm.eadventure.adventureeditor.data.chapterdata.scenes.Slidescene;
import es.eucm.eadventure.adventureeditor.data.chapterdata.scenes.Videoscene;
...
public class TimersListDataControl extends DataControl {
	...
	private TimerDataControl findDataControl(DataControl dataControl){
		int index = findDataControlIndex(dataControl);
		TimerDataControl theTimer = null;
		if (index != -1)
			theTimer = timersDataControlList.get( index );
		return theTimer;
	}
	...
	public void updateFlagSummary( FlagSummary flagSummary ) {
		...
		for( TimerDataControl timerDataControl : timersDataControlList )
			timerDataControl.updateFlagSummary( flagSummary );
	}
