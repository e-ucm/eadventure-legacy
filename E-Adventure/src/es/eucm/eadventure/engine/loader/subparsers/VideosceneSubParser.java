...
/**
 * Class to subparse videoscenes
 */
public class VideosceneSubParser extends SubParser {
    ...
    /**
     * Stores the videoscene being parsed
     */
    ...
    public void startElement( String namespaceURI, String sName, String qName, Attributes attrs ) {
        ...
        if( subParsing == SUBPARSING_NONE ) {
            ...
            // If it is a videoscene tag, create the videoscene with its id
            if( qName.equals( "videoscene" ) ) {
                ...
                for( int i = 0; i < attrs.getLength( ); i++ ) {
                    if( attrs.getQName( i ).equals( "id" ) )
                        cutsceneId = attrs.getValue( i );
                    if( attrs.getQName( i ).equals( "start" ) )
                        initialScene = attrs.getValue( i ).equals( "yes" );
                }
    
                videoscene = new Videoscene( cutsceneId );
                videoscene.setInitialScene( initialScene );
            }
    ...
    public void endElement( String namespaceURI, String sName, String qName ) {
        ...
        if( subParsing == SUBPARSING_NONE ) {
            ...
            // If it is a resources tag, add it to the videoscene
                ...
                videoscene.addResources( currentResources );
                reading = READING_NONE;
            }
            
            // If it is a name tag, add the name to the videoscene
            else if( qName.equals( "name" ) ) {
                videoscene.setName( currentString.toString( ).trim( ) );
            }
    
            // If it is a next-scene tag, add the next scene to the videoscene
            else if( qName.equals( "next-scene" ) ) {
                videoscene.addNextScene( currentNextScene );
                reading = READING_NONE;
            }
