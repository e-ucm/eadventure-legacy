...
/**
 * The walking to examine animation for the player
 */
public class PCWalkingToExamine extends PCState {

    /**
     * Creates a new WalkingToExamine
     * @param player the reference to the player
     */
    public PCWalkingToExamine( FunctionalPlayer player ) {
        super( player );
    }
    ...
    public void update( long elapsedTime ) {
        if( ( player.getSpeedX( ) > 0 && player.getX( ) < player.getDestX( ) - player.getWidth( ) ) || ( player.getSpeedX( ) <= 0 && player.getX( ) >= player.getDestX( ) + player.getWidth( ) ) )
            ...
            player.setState( FunctionalPlayer.EXAMINE );
    }
