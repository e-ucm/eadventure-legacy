...
public class Inventory {
     ...
     * shown)
     */
    private double dy = INVENTORY_PANEL_HEIGHT - 15.0;

    /**
     * Relative X coordinate of the inventory (Used for the movement of the
     * inventory to left-right)
     */
     ...
     * 
     * @param upper
     *            If the inventory is the upper one
     */
    ...
    public void draw( Graphics2D g ) {
        ...
        if( lastItem > Game.getInstance( ).getInventory( ).getItemCount( ) )
            lastItem = Game.getInstance( ).getInventory( ).getItemCount( );
     ...
     * 
     * @param dy
     *            Vertical position of the inventory
     */
     ...
     * 
     * @param dx
     *            Horizontal position of the inventory
     */
     ...
     * 
     * @param elapsedTime
     *            Time betweens updates
     */
    public void update( long elapsedTime ) {

        if( dx > 0.0 )
            setDX( dx + elapsedTime / 5.0 );
        if( dx < 0.0 )
            setDX( dx - elapsedTime / 5.0 );
    }
